...
//     // Back substitution
//     int * coefs = (int*)malloc(n * sizeof(int));
//     for (int i = n - 1; i >= 0; i--) {
//         coefs[i] = vector[i];
//         for (int j = i + 1; j < n; j++) {
//             coefficients[i] = (int) mod(coefs[i] - mod(matrix[i][j] * coefficients[j]));
//         }
//         coefficients[i] = (int) mod(coefficients[i] * modInverse(matrix[i][i]));
//     }
...
//     polynomialCoefficients(points, numPoints, coefficients, degree);


//     for(int i = 0; i < numPoints; i++){
//         if(check(points[i], coefficients, degree) == 0){
//             printf("Error: Point %d is not on the polynomial\n", i);
//         }
//     }
